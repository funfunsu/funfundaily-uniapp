...
const drawerRef = ref(null); // 用于访问子组件实例
...
// ✅ 任务列表事件 - 只保留逻辑，无需再定义isTaskSelected，已移入组件
...
async function handleMemberChange(e) {
  currentMember.value = e.currentMember; currentGroup.value = e.currentGroup; await fetchAllData()
}
...
async function fetchTaskList() {
  try {
    ...
    console.error('获取任务失败', error); taskList.value = []
  }
}
